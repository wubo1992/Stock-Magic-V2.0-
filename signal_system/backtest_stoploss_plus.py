...
def run_backtest():
    ...
    trades = []
    pending = {}  # {symbol: {'date': buy_date, 'shares': shares, 'avg_price': avg_price}}
    ...
    print("\n" + "="*60)
    print("次日开盘买卖策略回测结果（Stop Loss Plus版）")
    print("="*60)
    ...
    # 保存
    Path('output/v_stoploss_plus').mkdir(parents=True, exist_ok=True)
    df.to_csv('output/v_stoploss_plus/trades.csv', index=False)
    print("\n已保存到 output/v_stoploss_plus/trades.csv")
